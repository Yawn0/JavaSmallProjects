...
public class PianoDiStudio {
     ...
     * @exception PianoDiStudioException eccezione che puo' avvenire in caso di lista vuota<br>
     *                                  o se viene cercato un nodo inesistente
     */

    private Nodo getLinkPosizione(int posizione) throws PianoDiStudioException {
        int n = 1;
        Nodo p = head;
        if (head == null)
            throw new PianoDiStudioException("Lista vuota");
        if ((posizione > elementi) || (posizione < 1))
            throw new PianoDiStudioException("Posizione errata");
     ...
     *
     * @exception PianoDiStudioException errore possibile se la lista é vuota
     */

    private void eliminaInTesta() throws PianoDiStudioException {
        if (head == null)
            throw new PianoDiStudioException("Lista vuota");
     ...
     * @exception PianoDiStudioException errore possibile se l'esame da eliminare non esiste
     */
    public void eliminaEsame(int posizione) throws PianoDiStudioException {
        if (posizione == 1)
            eliminaInTesta();
            ...
            try {
                ...
                elementi--;
            } catch (PianoDiStudioException e) {
